...
**Cron strategy.** No `setInterval` loops — Next.js / Vercel doesn't keep long-lived processes. Background work runs as route handlers under `app/api/cron/<name>/route.ts`, each exporting **both `GET` and `POST`** (schedulers fire `GET`) and gated by `Authorization: Bearer ${CRON_SECRET}`. Targets: `outbox-drain` (1m), `email-queue-drain` (1m), `verification-cleanup` (hourly), `order-expiration` (5m), `purchase-reconcile` (5m — **load-bearing**, see below), `webhook-log-purge` (daily), `email-job-purge` (daily — purges SENT EmailJob rows older than `EMAIL_JOB_RETENTION_DAYS`, default 30).

⚠️ **Two schedulers, one inventory.** Vercel Hobby only accepts daily crons and **rejects the whole deployment** if `vercel.json` declares anything more frequent. So `vercel.json` carries only the 2 daily jobs; the 5 frequent ones are called over HTTP from cron-job.org. The source of truth for all 7 is [frontend/cron-schedule.json](frontend/cron-schedule.json) (`trigger: 'vercel' | 'external'`), not `vercel.json` — [cron/vercel-verb.test.ts](frontend/src/lib/server/cron/vercel-verb.test.ts) asserts the two files agree, that no frequent job leaks back into `vercel.json`, and that every job exports `GET`. Adding a cron means adding it to the manifest **and**, if it is sub-daily, creating it on cron-job.org — see [CRON.md](CRON.md). A job that exists only in the third-party web UI is invisible to everyone reading this repo.

⚠️ `purchase-reconcile` is not a safety net. Maketou — the Togolese mobile-money provider that sells credits — has **no webhook of any kind**: its whole API is `POST /cart/checkout` + `GET /cart/{id}`. Buyers confirm in their operator's app and rarely return to the tab, so most payments are confirmed *after* the browser stops polling. Delete this cron — or forget to recreate it on cron-job.org — and paid credits are silently never delivered. Multi-instance coordination still uses [frontend/src/lib/server/leader-lease.ts](frontend/src/lib/server/leader-lease.ts) Redis leases where two crons could collide. The Bictorys charge `CircuitBreaker` is still in-memory single-instance — replace with a Redis-backed variant for multi-pod prod (documented limitation).
...
- **Deoflow inverts the starter's signup default.** Email verification is **OFF** unless `AUTH_REQUIRE_EMAIL_VERIFICATION="1"` — owner decision, 2026-08-14. Signup creates an already-verified user and issues the session on the spot; login skips the verified-email gate. Rationale: no email provider is wired in production (`RESEND_API_KEY` absent), so the old default left signup waiting on a code that never arrived, silently. **What this costs:** signup now sets cookies for a new email and not for a taken one, so email existence is observable (enumeration), and nobody proves they own the address they register. Re-enable with the env var once Resend is live — no code change, the two-step flow is intact and `src/app/signup/page.tsx` branches on the server's response. See [frontend/src/lib/server/auth/email-verification.ts](frontend/src/lib/server/auth/email-verification.ts).
